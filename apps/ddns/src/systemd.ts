...
const CONFIG_ENV = `${APP_NAME.toUpperCase().replace(/-/g, "_")}_CONFIG`

/** The only directories `teardown --purge` is allowed to delete recursively. */
const OWNED_CONFIG_DIRS = [resolve(`/etc/${APP_NAME}`), resolve(join(homedir(), ".config", APP_NAME))]

const systemUnitPath = `/etc/systemd/system/${APP_NAME}.service`
const userUnitPath = join(homedir(), ".config", "systemd", "user", `${APP_NAME}.service`)

const unitPath = (scope: ServiceScope): string => (scope === "system" ? systemUnitPath : userUnitPath)
const systemctl = (scope: ServiceScope, args: string[]): string[] => (scope === "system" ? args : ["--user", ...args])
...
const requireScopePrivileges = (scope: ServiceScope, action: string): void => {
    if (scope === "system" && !isRoot()) {
        throw new FlowKitError(
            ...
            `either \`sudo ${APP_NAME} ${action}\`, or \`${APP_NAME} ${action} --user\` which needs no root at all`,
        )
    }
    if (scope === "user" && isRoot()) {
        throw new FlowKitError(
            `${action} --user as root would install into root's home`,
            `drop the sudo, or use \`sudo ${APP_NAME} ${action}\` for a system service`,
        )
    }
}
...
[Service]
Type=simple
${account}Environment=${CONFIG_ENV}=${configPath}
...
export const setupService = (options: SetupOptions): void => {
    const { scope } = options
    ...
    if (scope === "system") {
        ...
        // let the router POST its NOTIFY back to us
        if (ufwIsActive()) {
    ...
    run("systemctl", systemctl(scope, ["enable", "--now", `${APP_NAME}.service`]))
    write("info", `installed ${target} and started ${APP_NAME}`)

    if (scope === "user") {
        const linger = tryRun("loginctl", ["show-user", process.env["USER"] ?? "", "-p", "Linger", "--value"])?.trim()
        if (linger !== "yes") {
            write(
                "warn",
    ...
    write("info", `follow it with: journalctl ${scope === "user" ? "--user " : ""}-u ${APP_NAME} -f`)
}
...
export const teardownService = (options: TeardownOptions): void => {
    const { scope } = options
    requireScopePrivileges(scope, "teardown")

    tryRun("systemctl", systemctl(scope, ["disable", "--now", `${APP_NAME}.service`]))
    ...
    run("systemctl", systemctl(scope, ["daemon-reload"]))
    tryRun("systemctl", systemctl(scope, ["reset-failed", `${APP_NAME}.service`]))

    if (scope === "system" && ufwIsActive()) {
        const gateway = tryRun("ip", ["route", "show", "default"])?.trim().split(/\s+/)[2]
        if (gateway) {
            tryRun("ufw", [
                "delete",
    ...
    if (OWNED_CONFIG_DIRS.includes(configDir)) {
        if (existsSync(configDir)) {
            rmSync(configDir, { recursive: true, force: true })
            write("info", `removed ${configDir}`)
        }
    } else {
        write("info", `left ${configDir} in place - not a directory ${APP_NAME} created`)
    }
...
export const serviceStatus = (scope: ServiceScope): string =>
    tryRun("systemctl", systemctl(scope, ["status", `${APP_NAME}.service`, "--no-pager"])) ??
    `${APP_NAME}.service is not installed (${scope} scope)`

export const controlService = (scope: ServiceScope, action: "start" | "stop" | "restart"): void => {
    requireScopePrivileges(scope, action)
    run("systemctl", systemctl(scope, [action, `${APP_NAME}.service`]))
    write("info", `${action}ed ${APP_NAME}`)
}
